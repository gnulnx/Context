"""Pinned model download, ownership, validation, and installer progress."""
...
from bl_context.embedding_runtime import probe
...
def host(monkeypatch):
    ...
    monkeypatch.setattr(
        requests,
        "get",
        lambda url, **kwargs: get(
            f"http://127.0.0.1:{server.server_port}/model", **kwargs
        ),
    )
    try:
        yield state, payload
...
def test_interrupted_http_download_resumes_and_checks_hash(tmp_path, host):
    ...
    updates = []
    embedding.download(
        target, len(payload), hashlib.sha256(payload).hexdigest(), updates.append
    )
    assert state["ranges"][0] == 0
...
def test_range_ignored_and_rate_limit_preserves_partial(tmp_path, host):
    ...
    with pytest.raises(RuntimeError, match="rate limited"):
        embedding.download(
            target, len(payload), hashlib.sha256(payload).hexdigest(), lambda value: None
        )
    assert state["ranges"] == [100]
...
def test_checksum_failure_never_publishes(tmp_path, host):
    ...
    with pytest.raises(RuntimeError, match="checksum"):
        embedding.download(tmp_path / "model.onnx", len(payload), "0" * 64, lambda value: None)
...
def test_unconfigured_model_never_downloads_and_uninstall_clears_selection(monkeypatch):
    ...
    monkeypatch.setattr(requests, "get", lambda *args, **kwargs: pytest.fail("network"))
...
def test_partial_setup_is_owned_and_purge_preserves_unknown():
    storage.install()
    paths = storage.locations()
...
def test_probe_rejects_unusable_vectors():
    class Broken:
        def passage_embed(self, texts):
            return [[0.0] * embedding.DIMENSIONS] * 3
...
def test_json_install_has_no_progress_output():
    result = CliRunner().invoke(
        main, ["install", "codex", "--step", "embedding_model", "--json"]
    )
    assert result.exit_code == 1
    assert json.loads(result.output)["checks"][-1]["step_id"] == "embedding_model"


def test_non_terminal_progress_stays_inside_installer_output():
    result = CliRunner().invoke(
        main, ["install", "codex", "--step", "embedding_model", "--no-color"]
    )
    assert result.exit_code == 1
    heading = result.output.index("Base Layer Context")
    progress = result.output.index("Checking cached model")
    check = result.output.index("Embedding model available")
    assert heading < progress < check


@pytest.mark.skipif(os.environ.get("BLCTX_INDEX_TEST") != "1", reason="Actual pinned CPU model")
def test_real_model_offline_lifecycle(install_embedding, monkeypatch):
    storage.install()
    install_embedding()
    monkeypatch.setattr(requests, "get", lambda *args, **kwargs: pytest.fail("network"))
    assert embedding.verify().startswith(embedding.MODEL)
    storage.uninstall()
    storage.install()
    embedding.install()
    assert embedding.verify().startswith(embedding.MODEL)
