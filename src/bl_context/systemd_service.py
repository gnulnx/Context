"""Linux systemd user service backend; all operations are installation-scoped."""
import fcntl
import os
import stat
import subprocess
...
DIAGNOSTICS = 'Inspect journalctl --user for daemon logs.'
...
def manager(*args):
    try:
        ...
        raise RuntimeError('User service manager unavailable; use a Linux systemd user session with a session bus.') from exc
